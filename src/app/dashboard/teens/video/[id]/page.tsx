		...
		if (videoRef.current) {
			...
			try {
				...
				if (video.videoWidth === 0 || video.videoHeight === 0) {
					setError(
						"Video not fully loaded. Please wait and try again."
					);
					return;
				}
				...
				try {
					...
					setError(
						"Error creating image from video: " +
					...
					{videoData && (
						<>
							<div className="relative">
								...
								></video>
								<div className="absolute top-4 right-4 text-xs bg-black/60 text-white p-2 rounded-md">
									Pause video to analyze the current frame
									with AI
								</div>
							</div>
							<div className="mt-4">
								...
								<p className="mt-1 text-gray-700 line-clamp-4 overflow-x-auto">
									Video Prompt: {videoData.prompt}
								</p>
							</div>
						</>
					)}
