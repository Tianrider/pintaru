...
export default function VideoCard({videoData}: VideoCardProps) {
	...
	> | null>(null);
	const {is_ready, title, video_url, thumbnail_url, subject, created_at, id} =
		videoData;
	...
	useEffect(() => {
		...
		const fetchInitialData = async () => {
			try {
				...
				console.error(
					`Error fetching initial data for video ${id}:`,
					error
				);
			}
		};
				...
				(payload) => {
					if (payload.new) {
						...
						const updatedProgress =
							typeof payload.new.progress === "number"
								? payload.new.progress
								: 0;
	...
	const cardClasses =
		"bg-white rounded-xl shadow-sm p-4 border-[1px] flex flex-col hover:shadow-md transition-all duration-200 relative overflow-hidden group";

	const router = useRouter();

	return (
		<div
			className={`${cardClasses} h-auto min-h-[18rem] md:h-72 cursor-pointer`}
			onClick={() => router.push(`/dashboard/teens/video/${id}`)}
		>
			<div
			...
			<div className="flex items-center justify-between mt-auto pt-2">
				<span className="px-3 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded-full truncate max-w-[60%]">
					{subject}
				</span>
