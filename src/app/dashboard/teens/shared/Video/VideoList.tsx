...
const VideoList = forwardRef<VideoListRef>((props, ref) => {
	...
	const subjects = useMemo(() => {
		...
		const uniqueSubjects = Array.from(
			new Set(data.map((video: VideoDataType) => video.subject))
		);
	...
	const filteredVideos = useMemo(() => {
		...
		return data.filter((video: VideoDataType) => {
			const matchesSearch =
				...
				video.subject.toLowerCase().includes(search.toLowerCase());

			const matchesSubject =
				selectedSubjects.length === 0 ||
				selectedSubjects.some(
					(subject) => subject.value === video.subject
				);
			...
			<div className="flex gap-4 max-w-4xl mx-auto w-full flex-wrap md:flex-nowrap">
				<div className="w-full md:w-[70%] relative h-12">
					<Input
						...
						className="rounded-full h-full px-11 border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 focus:border-secondary-yellow text-base"
						value={search}
				...
				<div className="w-full md:w-[30%]">
					<div className="relative">
						<MultiSelect
							...
							className="w-full rounded-full shadow-sm flex px-2 items-center hover:shadow-md transition-all duration-200 text-base h-auto min-h-12"
							badgeClassName="my-1"
						/>
			...
			<div className="mt-2">
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 grid-rows-2">
					{isLoading ? (
						...
						paginatedVideos.map((video: VideoDataType) => (
